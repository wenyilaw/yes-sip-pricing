...
export default function SIPPricingPlaybook() {
  ...
  const getWatermarkOverlay = (enabled = true) => (watermark.enabled && enabled ? (
    <div aria-hidden="true" style={{
      position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center',
      pointerEvents: 'none', userSelect: 'none', overflow: 'hidden', zIndex: 0
    }}>
      <div style={{
        fontSize: watermark.fontSize, fontWeight: 800, letterSpacing: '0.08em', whiteSpace: 'nowrap',
        color: watermark.color, opacity: watermark.opacity, transform: `rotate(${watermark.rotation}deg)`
      }}>{watermark.text}</div>
    </div>
  ) : null);
